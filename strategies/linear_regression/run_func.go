...
func RunLinearRegression(userID int, rawConfig []byte, test int, sessionID *int) ([]*trade.Trade, error) {
	...
	if test == testing.Disable {
		inst, monitorChannel, keys, err = actions.PrepareStrategy(config.BaseStrategyConfig, userID, 9)
	} else if test == testing.BackTest{
		inst, replayMonitor, keys, err = actions.PrepareBackTesting(config.BaseStrategyConfig, *sessionID, userID, 9)
