...
func RunSimpleRSI(userID int, rawConfig []byte, test int, sessionID *int) ([]*trade.Trade, *int, error) {
	...
	if test == testing.Disable {
		inst, monitorChannel, keys, err = actions.PrepareStrategy(config.BaseStrategyConfig, userID, 6)
	} else if test == testing.BackTest{
		inst, replayMonitor, keys, err = actions.PrepareBackTesting(config.BaseStrategyConfig, *sessionID, userID, 6)
	...
	strat, err := NewSimpleRSI(monitorChannel, config, keys, nil, inst)
	if err != nil {
		return nil, nil, err
	}
	if test == testing.Disable {
