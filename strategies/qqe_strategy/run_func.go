...
func RunQQEStrategy(userID int, rawConfig []byte, test int, sessionID *int) ([]*trade.Trade, *int, error) {
	...
	if test == testing.Disable {
		inst, monitorChannel, keys, err = actions.PrepareStrategy(config.BaseStrategyConfig, userID, 7)
	} else if test == testing.BackTest{
		inst, replayMonitor, keys, err = actions.PrepareBackTesting(config.BaseStrategyConfig, *sessionID, userID, 7)
	...
	strat, err := NewQQEStrategy(monitorChannel, config, keys, nil, inst)
	if err != nil {
		return nil, nil, err
	}
	if test == testing.Disable {
